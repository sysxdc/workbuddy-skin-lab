...
- 页面控制面板：可拖动的 `🎨` 可预览、保存和切换已保存主题，即时换背景并调整显示效果；
- 多主题管理：新主题不会覆盖旧主题，可在控制面板中随时切换；
- 三背景候选：新生成主题默认保存三张背景，方案1自动启用，另外两张可在 `🎨` 即时切换；
- 三套文案：专注、轻松、活力循环切换，手动修改按主题和套装独立保存；
...
| 保存当前主题（下次启动） | 把当前预览主题设为下次“开始使用”时自动恢复的主题 |
| 背景缩略图 | 在当前主题的1–3张背景间即时切换并重新匹配配色，不产生生图调用 |
| 文案设置 | 在专注、轻松、活力三套文案间循环，修改内容按套装隔离 |
...
控制面板的下拉切换只做即时预览；点击“保存当前主题（下次启动）”后才会记录启动偏好。下一次运行“开始使用”会通过固定 CDP 读取该安全主题 ID，确认主题目录仍有效后同步到磁盘设置。显式 `apply --theme <id>` 始终优先。页面控制面板临时上传的背景保存在 WorkBuddy 渲染页的 `localStorage` 中。AI 生成并接受的主题则永久保存在
...
| `backgrounds` | 1–3张候选背景；第一项与 `background` 一致 | 每项为 `{id,label,asset}`，逐张校验，合计硬上限60 MB |
...
| `copySets` | 可选文案套装 | 最多3套；首页文字和固定模块文字，不接受 HTML |
...
显式维护：`sidebar`、`topbar`、`detail-panel`、`main-content`、`home-composer`、`home-stage`、`quick-actions`、
...
新增流程会把当前范例作为唯一布局模板：三张候选背景（直接使用图片时为一张）、一个欢迎横幅装饰、三个保留“办公 / 开发 / 创意”
...
| 素材 | API 请求 | 最终文件 |
|---|---|---|
| 背景 | 一个前台批次并行执行3次 `n=1`，每张 2048×1152 | 三张 2048×1152 JPEG，每张建议≤12 MB，合计硬上限60 MB |
| 欢迎横幅装饰 | 1024×1024 | 透明 PNG，≤4 MB |
| 三个场景图标 | 各 1024×1024 | 各 1024×1024 透明 PNG，≤4 MB |
...
半自动流程支持提示词生成、参考图生成和直接使用参考图。开始前只做一次完整调用量确认：前两种为三次单图背景调用加五次模块调用（共8次），直接模式只有五次模块调用。三次背景调用在一个前台命令中并行等待，避免 `gpt-image-2` 忽略 `n=3` 时只返回一张。
三张背景经 `preview` 展示后默认使用方案1并继续，不再等待背景确认；切换已有方案不会调用 API。每次生图会提前提示预计等待1–10分钟。
五张派生素材通过一个可追溯的前台命令并行生成，命令每15秒输出一次不含敏感信息的保活消息，窗口会等待全部结果。即使工具返回任务句柄，Skill 也必须在同一轮持续等待，不能以“等待自动通知”结束；用户不需要反复询问进度。失败不会自动重试。
改文字不产生生图请求，重做背景或单个图标都必须重新明确授权对应次数。本地参考图
...
“继续”；除上传、完整调用量、额外重试和最终接受外，Skill 应在同一轮连续执行到下一个确认点。
...
```text
release\WorkBuddy-Skin-Lab-<版本>-Windows.zip
```
