...
为 WorkBuddy 桌面端更换背景，并提供背景安全区、任务页展示方式和回答阅读层等显示保护。当前稳定版本为 **v1.3.1，仅背景模式**；本分支正在预览下一版本的环境特效与顶部图文层。
...
- 在原生顶部栏下方添加自定义图案和最多 60 个字符的纯文本；
- 保存当前主题，供下次启动恢复；
- 重置当前主题的本机覆盖，或恢复 WorkBuddy 原生界面。

天气粒子只叠加在背景层；顶部图文使用独立的非交互显示层，并保持在系统弹窗下方。两者都不会接收点击，也不会替换 WorkBuddy 的原生文字和控件。系统启用“减少动态效果”时，天气动画会自动停止。

本项目不会再提供宠物、原生组件动效、首页文字替换、场景图标替换或装饰组件功能。

## 功能分支预览：天气特效与顶部图文

> 该功能只存在于 `codex/weather-header-overlays` 开发分支，v1.3.1 Release 暂不包含。
...
4. “顶部文字”留空即隐藏，最多 60 个字符；
5. “顶部位置”可选左侧、居中或右侧，“顶部大小”可选小、中、大；
6. “选择顶部图案”支持 PNG、JPEG、WebP、GIF 和 SVG，“清除图案”只清除顶部小图，不影响背景。

这些设置按当前主题单独保存。切换主题会恢复该主题自己的设置；“重置本主题”会同时清除天气和顶部图文设置。
