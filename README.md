...
- 打开或关闭回答阅读层；
- 添加下雨、雷雨或下雪背景特效，并选择轻/中/强三档；
- 在原生顶部栏下方添加自定义图案和最多 60 个字符的纯文本；
- 保存当前主题，供下次启动恢复；
- 重置当前主题的本机覆盖，或恢复 WorkBuddy 原生界面。

天气和顶部图文只叠加在背景层，不接收点击，也不会替换 WorkBuddy 的原生文字和控件。系统启用“减少动态效果”时，天气动画会自动停止。
...
1. “天气特效”可选关闭、下雨、雷雨或下雪；
2. “天气强度”可选轻、中、强；
3. “顶部文字”留空即隐藏，最多 60 个字符；
4. “顶部位置”可选左侧、居中或右侧；
5. “选择顶部图案”支持 PNG、JPEG、WebP、GIF 和 SVG，“清除图案”只清除顶部小图，不影响背景。
