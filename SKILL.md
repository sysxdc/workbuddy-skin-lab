...
description: 在 Windows WorkBuddy 内生成、应用、保存和切换背景主题。只处理背景图片，不修改首页文字，不生成或注入装饰组件；只通过 127.0.0.1 CDP 工作。
allowed-tools: Read, Write, Bash
---

# WorkBuddy Skin Lab（仅背景模式）
...
1. 运行 `doctor`、`validate`、`status` 和 `theme-generation-job.mjs resume`。有旧作业时仅恢复 `background-v1` 作业；旧版 `home-scene-v1` 作业不得继续生成模块，应新建背景作业。
