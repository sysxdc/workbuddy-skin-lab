...
3. 开始前一次说明完整调用量并取得确认：generate/edit 为“3次单图背景调用 + 5次模块调用，共8次”；direct 为“0次背景调用 + 5次模块调用”。三次背景调用由一个前台命令并行完成，不能把它说成一次 API 调用。
4. 所有图片固定 `gpt-image-2`、`quality=low`、`response_format=url`。背景 2048×1152；其余请求 1024×1024。提示词必须禁止文字、UI框架、WorkBuddy标志和水印。
5. 用一个 `run-backgrounds` 前台批次并行执行三次 `n=1`，取得三张背景。部分失败时保留成功候选，只在用户明确授权缺少次数后补齐；不得自动重试。默认选择方案1，不等待背景确认；运行 `preview` 展示三张或地址后继续。
