...
点击 `🎨` 可以切换主题、背景候选、浅深色、背景安全区、任务页背景和回答阅读层。使用 `main` 源码版时，还可以在“环境粒子特效”中选择雨、雪、爱心、星星或自定义符号。点击“保存当前主题（下次启动）”后，下次启动会自动恢复。
...
这些功能依赖 WorkBuddy 内部页面结构，软件更新后容易错位或丢失，已从 1.3.1 的 Skill 和运行时中删除。顶部图案和顶部文字也已移除；背景粒子不替换原生组件，可以独立使用。
