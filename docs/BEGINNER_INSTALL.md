# WorkBuddy Skin Lab v1.5.2：小白安装使用教程

这是一份讲“下载、解压、检查、启动、换背景、粒子特效和恢复”的图文教程。照着做，不需要懂代码。

> v1.5.2 只扩展背景和粒子层：不替换 WorkBuddy 的文字、图标、卡片、宠物或其它组件，避免 WorkBuddy 更新后出现错位和内容丢失。
...
打开 [v1.5.2 发布页](https://github.com/sysxdc/workbuddy-skin-lab/releases/tag/v1.5.2)，下载：

```text
WorkBuddy-Skin-Lab-1.5.2-Windows.zip
```
...
7. 在“环境粒子特效”中选择雨、雷雨、雪、爱心、星星、手输符号或 AI 透明 PNG 素材，并调整强度、速度和颜色。
...
- **粒子特效**：可选关闭、下雨、雷雨、下雪、冒爱心、下星星、手输符号或 AI 透明 PNG 素材。
- **粒子强度**：性能较弱的电脑建议选择“轻”。
- **粒子速度**：可选慢、正常或快。
- **粒子颜色**：点击颜色框自由选择。
- **手输符号（文字）**：最多两个 Unicode 字符，例如 `🌸`、`♫` 或 `✨`；只能输入文字/Emoji，不能用于 AI 图片。
- **AI 透明 PNG 素材**：安装两个 Skill 后，可生成三张透明 PNG 候选；确认后会创建并应用用户派生主题，自动启用 AI 素材。点击缩略图可切换候选，并调整飘落/上浮/漂浮/横向掠过、摆动、旋转、缩放起伏和闪烁。

粒子位于背景层，不接收点击，不会遮挡 WorkBuddy 的按钮、输入框和弹窗。Windows 开启“减少动态效果”时，动画会自动停止。设置按主题保存，“重置本主题”会清除该主题的粒子设置。

不同粒子有各自的自然轨迹：雪花左右飘落，爱心摇摆上浮，星星闪烁下落，手输符号自由漂浮。粒子位置、延迟和漂移会独立打散，不会再排成规则斜线。
...
这是 v1.5.2 的预期行为。旧功能依赖 WorkBuddy 内部页面结构，更新后无法可靠对齐，已经移除。旧主题的背景和内置粒子仍可使用。
