...
test("最终用户流程只在 WorkBuddy 对话中调用两个原生 Skill", async () => {
  ...
  assert.match(skill, /一个前台 `run-derived`/);
  assert.match(skill, /不能.*让用户反复发送/);
  assert.match(skill, /同一轮.*wait\/read/);
  assert.match(skill, /保存当前主题（下次启动）/);
  ...
  assert.match(setup, /WorkBuddy-Skin-Lab-1\.3\.0-Skill\.zip/);
