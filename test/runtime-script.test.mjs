...
test("生成的注入脚本只保留背景控制和可清理状态", () => {
  ...
  assert.match(script, /--wb-topbar-offset/);
  assert.match(script, /topbarRect\.bottom - appRect\.top/);
...
test("CSS 使用 WorkBuddy 稳定锚点", () => {
  ...
  assert.match(css, /top:\s*var\(--wb-topbar-offset\)/);
