...
| 素材 | API请求 | 最终文件 |
|---|---|---|
| 背景候选 | 2048×1152，单次 `n=3` | 三张 2048×1152 JPEG，每张建议≤12 MB，合计硬上限60 MB |
...
3. 在开始前说明完整调用量，执行 `confirm --gate generation`：
   - generate/edit：一次三输出背景请求 + 五次模块请求。
   - direct：零次背景请求 + 五次模块请求。
4. generate/edit 运行一次 `run-image --role background`，必须正好得到三条公开 HTTPS URL；direct 直接 `ingest`。
5. `ingest --role background` 下载并标准化全部候选。任一缺失、下载失败或标准化失败就停止，不补图、不自动重试。
...
node scripts/theme-generation-job.mjs run-image --job <jobId> --role background --prompt-file <提示词> --skill-script <nonelinear脚本>
...
单项重做前用 `authorize --job <jobId> --call <role>` 增加一次明确授权。背景重做仍是一整次 `n=3` 请求，并使旧候选和派生结果失效；切换已有背景不调用 API。
